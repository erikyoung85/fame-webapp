  ...
  getTeamDetails(
    ...
    return from(
      this.supabaseService.client
        .from('teams')
        .select(
          'id, season_year, name, schools(id, name, abbreviation, city, state, logo_url), sports(id, name, gender), roster_entries(id, position, jersey_number, athletes(id, first_name, last_name, gender))'
        )
        .eq('id', teamId)
        .single()
    );
